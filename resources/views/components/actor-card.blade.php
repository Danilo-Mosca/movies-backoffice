...
<div class="card shadow-sm mb-4" style="">
    ...
    <div class="card-body">
        ...
        <div class="mb-2 d-flex py-3 border-bottom">
            <a class="card-title fw-bolder" href="{{ route('actors.show', $actorID) }}">Visualizza scheda
                anagrafica...</a>
        </div>
        ...
        <div class="mt-4 text-end">
            <a href="{{ route('actors.edit', $actorID) }}"
            ...
            <button type="button" id="delete" data-bs-toggle="modal"
                data-bs-target="#modal-{{ $actorID }}">Elimina</button>
        </div>
    ...
    id="modal-{{ $actorID }}"
    e:
    aria-labelledby="exampleModalLabel-{{ $actorID }}" 
    --}}
    <div class="modal fade" id="modal-{{ $actorID }}" tabindex="-1"
        aria-labelledby="exampleModalLabel-{{ $actorID }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel-{{ $actorID }}"><strong>{{ $fullName }}</strong></h1>
                ...
                <div class="modal-footer">
                    ...
                    {{-- Per il DELETE non possiamo usare un link perchè i link chiamano sempre un metodo get, questo invece deve essere un metodo delete, allora lo facciamo tramite un form nascosto nel pulsante di conferma: --}}
                    <form action="{{ route('actors.destroy', $actorID) }}" method="POST">
                        @csrf
                        @method('DELETE')
